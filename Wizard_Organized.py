...
class Dragon:
    ...
    def draw(self, screen):
        ...
        elif 1500 <= self.boss_fight_timer <= 1880:
            self.facing = 'Left'
            self.dragon_X += self.direction
...
class Game:
    ...
    # Set variable to hold saved character or default character if first time opening the game
        ...
        self.boss_fight_duration = 60  # 1 minute of boss fight
        self.running_duration = 120  # 2 minutes of running
        self.game_duration = 0
        self.dragon = None
        self.fireball_spawn_timer = 0
        self.boss_fight_phase = 'dragon_animation'
    ...
    def running_state(self):
        ...
        if self.game_score.score % 400 == 0:
            self.transition = True 
    ...
    # Draw Dragon
        if self.boss_fight_phase == 'dragon_animation':
            if not self.dragon:
                self.dragon = Dragon(self.screen.get_width(), self.screen.get_height())
            
            self.dragon.draw(self.screen)
            
            if self.dragon.animation_complete:
                self.boss_fight_phase = 'fireball_spawning'
        
        ## Handle fireball spawning phase
        #elif self.boss_fight_phase == 'fireball_spawning':
        #    # Spawn fireballs periodically
        #    self.fireball_spawn_timer += 1
        #    if self.fireball_spawn_timer > random.randint(50, 150):
        ...
        #            y_position, 
        #            obstacle_type=ObstacleType.FIREBALL
        #        )
        #        self.obstacles.add(fireball)
        #        self.fireball_spawn_timer = 0
